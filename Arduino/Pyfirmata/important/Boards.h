...
#define TOTAL_ANALOG_PINS       8       // Total number of analog pins (A0-A7)
#define TOTAL_PINS              20      // Total GPIO pins (digital + analog)
#define TOTAL_DIGITAL_PINS      20      // Total number of digital pins
#define TOTAL_PORTS             2       // Number of ports (depends on microcontroller ports)
#define VERSION_BLINK_PIN       13      // Onboard LED is typically on pin 13

// Analog pins are part of digital pin numbering
#define analogInputToDigitalPin(p) ((p < TOTAL_ANALOG_PINS) ? (p + 14) : -1)

// Required macros
#define IS_PIN_DIGITAL(p)       ((p) >= 0 && (p) < TOTAL_DIGITAL_PINS)                 // Check if a pin is digital
#define IS_PIN_ANALOG(p)        ((p) >= 14 && (p) < (14 + TOTAL_ANALOG_PINS))          // Check if a pin can be analog
#define IS_PIN_PWM(p)           ((p) == 2 || (p) == 3 || (p) == 5 || (p) == 6)         // Define pins supporting PWM
#define IS_PIN_I2C(p)           ((p) == 18 || (p) == 19) // I2C capability: A4=SDA=18, A5=SCL=19

// Macro for pin remapping
#define PIN_TO_DIGITAL(p)       (p) // Digital pin mapping
#define PIN_TO_ANALOG(p)        ((p) - 14) // Maps analog pins A0-A7 to indices 0-7
#define PIN_TO_PWM(p)           (p) // Direct mapping for PWM pins

// Pin capabilities (custom-related capabilities can be mapped here)
#define PIN_CAPABILITY_ANALOG(o, p)
